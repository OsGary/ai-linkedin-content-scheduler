...
interface PostCardProps {
  post: Post
  onMove: (newStatus: PostStatus) => void
}
...
export function PostCard({ post, onMove }: PostCardProps) {
  ...
  const handleEdit = () => {
    // Will be implemented in the modal task
  ...
  const availableStatuses: PostStatus[] = ['idea', 'draft', 'scheduled', 'published']
  const currentIndex = availableStatuses.indexOf(post.status)
  const canMoveLeft = currentIndex > 0
  const canMoveRight = currentIndex < availableStatuses.length - 1
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {canMoveLeft && (
                <DropdownMenuItem onClick={() => onMove(availableStatuses[currentIndex - 1])}>
                  Move to {availableStatuses[currentIndex - 1]}
                </DropdownMenuItem>
              )}
              {canMoveRight && (
                <DropdownMenuItem onClick={() => onMove(availableStatuses[currentIndex + 1])}>
